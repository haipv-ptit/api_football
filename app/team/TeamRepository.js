...
class TeamRepository {
    ...
    static async getSeasonLeague(team_id, season_id) {
        let key = `:football_api:team_season_league:team_${team_id}_season_${season_id}`; 
        let resCached = await _findCachedByKey(key);
        if(resCached) {
            return resCached;
        }
        let resDb = await _getSeasonLeague(team_id, season_id);
        if(resDb) {
...
const _getSeasonLeague = async (team_id, season_id) => {
    let conn = db.getDb();
